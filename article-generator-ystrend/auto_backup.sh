...
# 変更があるか確認
if git diff --quiet && git diff --cached --quiet && [ -z "$(git ls-files --others --exclude-standard article-generator-ystrend/)" ]; then
    echo "----- バックアップ: 変更なし / スキップ -----" >> "$LOG_FILE"
    exit 0
fi

git add article-generator-ystrend/ >> "$LOG_FILE" 2>&1
