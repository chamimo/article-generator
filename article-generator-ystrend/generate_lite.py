        ...
        if FEATURES["article_type_mix"]:
            ...
            if dist.get("trend", 0) > 0:
                try:
                    ...
                    # allowed_themes フィルタをトレンドKWにも適用
                    if blog_cfg and blog_cfg.allowed_themes:
                        _at = [t.lower() for t in blog_cfg.allowed_themes]
                        _ng = [n.lower() for n in (blog_cfg.ng_keywords or [])]
                        before_t = len(trend_cands)
                        trend_cands = [
                            c for c in trend_cands
                            if any(t in c["keyword"].lower() for t in _at)
                            and not any(n in c["keyword"].lower() for n in _ng)
                        ]
                        log.info(f"[{blog_cfg.name}] トレンドKW テーマフィルタ: {before_t}件 → {len(trend_cands)}件")
