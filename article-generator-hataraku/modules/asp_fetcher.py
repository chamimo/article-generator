...
列構成:
  ...
  C列: 優先度（S / A / B / C）
  D列: アフィリリンク（URLまたはASP発行のHTMLタグをそのまま貼り付け可）
  E列: 訴求軸（SEO・CVポイント）

D列にHTMLタグ（<a href=...> や A8ネットのトラッキングタグ等）が入っている場合は
そのHTMLをそのまま記事に埋め込む。URLのみの場合は従来通り <a> タグを生成。
"""
...
def _is_html_tag(value: str) -> bool:
    """D列の値がURLではなくHTMLタグかどうかを判定する。"""
    return bool(_HTML_TAG_RE.search(value.strip()))


def _extract_href(html: str) -> str:
    """HTMLタグから最初の href 値を抽出する。見つからなければ空文字。"""
...
def fetch_asp_links(
    ...
    for row in rows[1:]:  # ヘッダー除外
        ...
        if _is_html_tag(raw_link):
            html_tag = raw_link
            url      = _extract_href(raw_link)  # href から URL を抽出（ログ・重複チェック用）
        else:
            html_tag = ""
            url      = raw_link
        ...
        results.append({
            "name":     name,
            "url":      url,
            "html_tag": html_tag,   # ASP発行HTMLタグ（あれば）
...
def build_asp_prompt_section(asp_list: list[dict]) -> str:
    ...
    D列にHTMLタグが入っている案件は「このHTMLをそのまま挿入すること」と指示する。
    """
    if not asp_list:
        return ""
    ...
    lines = [
        ...
        "- 【HTMLタグ指定あり】の案件は、指定されたHTMLを一切改変せずそのまま記事内に埋め込むこと",
        "- 【URLのみ】の案件は <a href=\"{URL}\" target=\"_blank\" rel=\"noopener noreferrer\">{案件名}</a> 形式で挿入",
        "",
    ]
    for i, item in enumerate(asp_list, 1):
        pri_label = next((k for k, v in _PRIORITY_MAP.items() if v == item["priority"]), "?")
        if item.get("html_tag"):
            line = (
                f"{i}. 【{item['name']}】[{pri_label}] 【HTMLタグ指定あり・改変禁止】\n"
                f"   以下のHTMLをそのまま記事内に挿入すること:\n"
                f"   {item['html_tag']}"
            )
        else:
