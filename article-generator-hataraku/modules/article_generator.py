...
# サイト情報
- サイト名: はた楽ナビ（https://hataraku-navi.com）
- テーマ: 転職・副業・キャリア情報メディア
- 対象読者: 転職・副業・キャリアアップを考えている方
- ライティングスタイル: 転職を複数回経験したWeb担当者の視点で、本音・リアルな情報を共感ベースで伝えるトーン

# 出力ルール
- WordPress SWELLの構造に完全準拠（独自CSSやstyle禁止）
- タイトルは30〜40字程度。キーワードを自然に含め、数字・メリット・疑問形などでクリックを促す（例：「転職エージェントおすすめ7選！初めての転職でも失敗しない選び方」）
- H2は最大3つ（すべてにキーフレーズを含める）
- H2見出しは疑問形だけでなく「断言・メリット提示・比較・方法提示」など自然に使い分ける。毎回「〜とは？」「〜できる？」にしないこと（例：✅「失業手当の正しい受け取り方と注意点」✅「おすすめ転職エージェント7選を徹底比較」❌「転職とは？」）
...
- 本文トーン: 転職経験者の本音・共感ベース。専門語はカッコで補足
...
- 結論ファーストな構成
- タグは最大5個（重要度の高いものを厳選）
- まとめチェックリストの直後に締めの文章（150〜200字）を1段落追加する。2〜3文構成で書くこと。構成例：①読者が感じているであろう迷いや苦労に共感する一文（「〜って、慣れるまでどれを選べばいいか本当に迷いますよね」など自然な表現で）→②やさしく背中を押す一文 →③締めの1文として「一つの情報に絞らず、いくつかの選択肢を見比べながら、自分に合った道を探してみてください。」のニュアンスで、口語的になりすぎずまとめ全体のトーンに馴染む言葉で締めくくる。「まず〇〇を試してみてください」「〇〇を選べば間違いありません」のような押しつけがましい表現は使わないこと。プロンプト末尾にASPアフィリリンクが提供されている場合のみ、文脈に合う1つを自然に組み込む（リンクのために文章を歪めない）。該当リンクがない場合はリンクなしでよい
...
<!-- wp:paragraph -->
<p>{{締めの文章（150〜200字・3文）。①読者の迷いや苦労に共感する一文 → ②やさしく背中を押す一文 → ③「一つの情報に絞らず、いくつかの選択肢を見比べながら自分に合った道を探してみてください」のニュアンスで締めくくる1文（口語的になりすぎず、まとめ全体のトーンに馴染む表現で）。「まず〇〇を試してみてください」のような押しつけ表現は禁止。ASPアフィリリンク提供済みのサービスが文脈に自然に合う場合のみ1つ挿入。}}</p>
<!-- /wp:paragraph -->
...
def generate_article(keyword: str, volume: int, differentiation_note: str = "",
    ...
    return _build_article(keyword, volume, differentiation_note,
                          ...
                          asp_list=asp_list, guide_links=guide_links, model=model)
